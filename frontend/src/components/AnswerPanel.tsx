...
export function AnswerPanel({ answer, streaming, model, searchTime, sources }: AnswerPanelProps) {
  ...
  return (
    <section className="glass-panel section-enter stagger-2 p-6">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <h3 className="font-display text-2xl text-ink">Synthesis</h3>
      ...
      <div className="prose prose-sm max-w-none prose-a:text-ember prose-headings:text-ink prose-p:text-ink/90 prose-li:text-ink/90">
        <ReactMarkdown remarkPlugins={[remarkGfm]}>{safeAnswer}</ReactMarkdown>
        {streaming ? <span className="inline-block h-5 w-1 animate-pulse rounded bg-ink/70 align-middle" /> : null}
      </div>
    </section>
  );
}
