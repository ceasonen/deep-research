...
export function SourceCard({ source, index }: SourceCardProps) {
  ...
  const readerUrl = source.pdf_url
    ? `/reader?pdf=${encodeURIComponent(source.pdf_url)}&title=${encodeURIComponent(source.title)}`
    : '';

  if (isArxiv) {
    return (
      <article className="rounded-2xl border border-ink/15 bg-white/70 p-4 shadow-soft">
        <p className="font-mono text-xs text-ink/60">
          [{index + 1}] arXiv {source.arxiv_id ? `· ${source.arxiv_id}` : ''}
        </p>
        <h4 className="mt-2 text-base font-semibold text-ink">{source.title}</h4>
        ...
        {source.categories?.length ? (
          <div className="mt-2 flex flex-wrap gap-1.5">
            {source.categories.slice(0, 5).map((category) => (
              <span key={`${source.url}-${category}`} className="rounded-full bg-sand px-2 py-1 text-[10px] text-ink/80">
                {category}
              </span>
            ))}
          </div>
        ) : null}
        <p className="mt-3 line-clamp-4 text-xs text-ink/80">{source.snippet}</p>
        ...
        <div className="mt-3 space-y-1 text-xs text-ink/80">
          {source.method_highlights ? <p><span className="font-semibold">Method:</span> {source.method_highlights}</p> : null}
          {source.limitations ? <p><span className="font-semibold">Limits:</span> {source.limitations}</p> : null}
          {source.reproduction_difficulty ? (
            <p>
              <span className="font-semibold">Repro:</span> {source.reproduction_difficulty}
            </p>
          ) : null}
          {source.published_date ? <p><span className="font-semibold">Published:</span> {source.published_date.slice(0, 10)}</p> : null}
        </div>
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          {source.pdf_url ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="rounded-full border border-ink/20 px-3 py-1.5 text-xs text-ink transition hover:border-ember"
            >
              Open PDF
            </a>
          ) : null}
